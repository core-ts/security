...
function handleToken(res, secret, name, t, verify, buildErr) {
  if (t.error) {
    ...
    return verify(t.token, secret).then(function (payload) {
...
function fromAuthorization(req, prefix) {
  ...
  if (data) {
    if (!data.startsWith(prefix)) {
      ...
      var token = data.substr(prefix.length);
      return { token: token };
    }
  }
  else {
  ...
  Handler.prototype.handle = function () {
    ...
    return function (req, res, next) {
      ...
      if (t.error) {
        ...
        _this.verify(t.token, _this.secret).then(function (payload) {
...
var MultiAuthorizer = (function () {
  function MultiAuthorizer(gt, privilege, buildErr, key, user, exact) {
        ...
        if (!t.end) {
          if (t.payload) {
            ...
            if (!obj) {
              ...
              if (exist(obj, v)) {
                var userId_1 = t.payload[_this.user];
                if (!userId_1) {
...
var Authorizer = (function () {
  function Authorizer(gt, privilege, buildErr, exact, key) {
    this.privilege = privilege;
    this.getToken = gt;
    this.buildError = (buildErr ? buildErr : buildError);
    this.key = (key ? key : 'id');
